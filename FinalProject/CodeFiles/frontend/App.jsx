...
const handlePyTorchTrain = async () => {
  ...
  try {
    ...
    console.log('Enviando datos al servidor Flask...');
    alert('Iniciando entrenamiento en el servidor...\n\nEsto puede tardar 1-3 minutos. Revisa la consola del servidor Flask para ver el progreso.');
    ...
    if (result.mae !== undefined) {
      setMlMetrics({
        mae: result.mae,
        rmse: result.mae * 1.3 // Aproximación
      });
      
      alert(`¡Entrenamiento completado! ✓\n\n` +
            `MAE: ${result.mae.toFixed(4)} cmH₂O\n` +
            `Muestras: ${result.samples.toLocaleString()}\n` +
            `Ciclos: ${result.breaths || 'N/A'}`);
    } else {
      alert('Entrenamiento completado pero no se recibieron métricas');
    }
  } catch (error) {
...
const handleKaggleSubmission = async () => {
  ...
  if (mlEpoch === 0) {
    alert('Primero debes entrenar el modelo con "Entrenar ML Real"');
    return;
  }
  ...
  try {
    ...
    console.log('Generando archivo de submission para Kaggle...');
    alert('Generando predicciones completas...\n\nEsto tomará 1-2 minutos. El archivo se descargará automáticamente.');
    
    const formData = new FormData();
    formData.append('file', file);
    
    const response = await fetch('http://localhost:5000/api/predict_and_download', {
      method: 'POST',
      body: formData
    });
    ...
    a.href = url;
    a.download = `submission_${new Date().getTime()}.csv`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
    
    alert('✅ Archivo de submission descargado!\n\n' +
          'El archivo contiene solo las columnas:\n' +
          '- id\n' +
          '- pressure\n\n' +
          'Listo para subir a Kaggle!');
    
  } catch (error) {
    console.error('Kaggle submission error:', error);
    alert('Error al generar submission: ' + error.message + '\n\n¿Está el servidor Flask corriendo?');
  }
};
...
const convertToCSV = (data) => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      // Parse CSV data
      const data = [];
      for (let i = 1; i < Math.min(lines.length, 15000); i++) { // Limit to 10000 rows
        const values = lines[i].split(',');
        if (values.length === headers.length) {
          const row = {};
          headers.forEach((header, idx) => {
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="flex gap-4 mb-6 flex-wrap">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:bg-gray-400"
              >
                Testear con machine learning
              </button>
              <button
              onClick={handleKaggleSubmission}
              disabled={!testDatasetLoaded || (mlEpoch === 0)}
